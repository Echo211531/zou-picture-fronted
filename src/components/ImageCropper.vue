...
// 确认裁剪,把blob类型转为file类型，然后传递给上传函数
const handleConfirm = () => {
...
const initWebsocket = () => {
  const pictureId = props.picture?.id    //传递图片id或弹窗可见才初始化连接
  ...
  websocket.on(PICTURE_EDIT_MESSAGE_TYPE_ENUM.ENTER_EDIT, (msg) => {
    ...
    message.info(msg.message)
    editingUser.value = msg.user //把当前正在编辑用户的值改为后端发来的用户
  })
  ...
  websocket.on(PICTURE_EDIT_MESSAGE_TYPE_ENUM.EDIT_ACTION, (msg) => {
    ...
    message.info(msg.message)
    switch (
      msg.editAction //后端发来具体用户的操作  执行前端对应的页面操作
    ) {
