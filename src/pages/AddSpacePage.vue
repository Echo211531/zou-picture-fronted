    ...
    <a-card title="空间级别介绍">
      <a-typography-paragraph>
        * 目前仅支持开通普通版，如需升级空间，请联系
        <a href="https://github.com/Echo211531" target="_blank">空说</a>。
      </a-typography-paragraph>
...
const handleSubmit = async (values: any) => {
  ...
  if (spaceId) {
    ...
    // 创建
    res = await addSpaceUsingPost({   //创建空间，返回空间id
      ...formData,
  ...
  if (res.data.code === 0 && res.data.data) {
    ...
    let path = `/space/${spaceId ?? res.data.data}`  //创建或编辑成功后，都要直接跳转到空间详情页
    router.push({
      path,
    })
  } else {
